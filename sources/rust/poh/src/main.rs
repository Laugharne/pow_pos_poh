...
fn build_block(previous_block_hash: String, current_transactions: &Block) -> String {
...
fn main() {
	...
	block_chain.iter_mut().for_each(|block| {
		let hash = build_block(
			previous_hash.clone(),
			block,
		);

		block.last_hash = previous_hash.clone();
		previous_hash   = hash;
